...
- **자동 매매 신호/추천에 대한 룰 (v1.1 업데이트)**:
  - 원칙: 정보 대시보드 위주. 시그널은 "표준 매매 룰에 따른 사실 기반 라벨"로 표기.
  - **사용자 명시 승인하에 "추천"/"위험" 라벨 카드 배지 허용** (2026-05-12):
    - 20일 외인+기관 동반 순매수 → 카드 초록 + "추천" 배지
    - 20일 외인+기관 동반 순매도 → 카드 빨강 + "위험" 배지
  - 단, **개별 매수가·손절가·익절가 단정 추천은 여전히 금지**. 손절선 가이드는 표준 룰(−5%/−7%) 정보 제공만.
  - 페이지 하단·헤더에 "본 라벨은 룰 기반 시그널 표시이며 매매 권유가 아님" 면책 유지.
  - 모든 매매 결정과 결과 책임은 사용자.
